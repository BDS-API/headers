class AABB;
class AABBBucket;
class AABBContactPoint;
class AABBShapeComponent;
class Abilities;
class Ability;
class AbilityCommand;
class AbsorptionMobEffect;
class AbstractArrow;
class ActionEvent;
class ActionQueue;
class ActivateToolDefinition;
class ActivateToolNode;
class ActivatorRailBlock;
class Actor;
class ActorAnimationController;
class ActorAnimationControllerGroup;
class ActorAnimationControllerGroupParseMetaData;
class ActorAnimationControllerInfo;
class ActorAnimationControllerPlayer;
class ActorAnimationControllerPtr;
class ActorAnimationControllerState;
class ActorAnimationControllerStatePlayer;
class ActorAnimationControllerStateTransition;
class ActorAnimationEvent;
class ActorAnimationGroup;
class ActorAnimationGroupParseMetaData;
class ActorAnimationInfo;
class ActorAnimationPlayer;
class ActorBlock;
class ActorBlockSyncMessage;
class ActorCommandOrigin;
class ActorComponent;
class ActorComponentFactory;
class ActorDamageByActorSource;
class ActorDamageByBlockSource;
class ActorDamageByChildActorSource;
class ActorDamageSource;
class ActorDefinition;
class ActorDefinitionAttribute;
class ActorDefinitionDescriptor;
class ActorDefinitionDiffList;
class ActorDefinitionEventSubcomponent;
class ActorDefinitionFeedItem;
class ActorDefinitionGroup;
class ActorDefinitionIdentifier;
class ActorDefinitionPtr;
class ActorEventCoordinator;
class ActorEventListener;
class ActorEventPacket;
class ActorFactory;
class ActorFactoryData;
class ActorFallPacket;
class ActorFilterGroup;
class ActorHasAbilityTest;
class ActorHasComponentTest;
class ActorHasDamageTest;
class ActorHasEquipmentTest;
class ActorHasMobEffect;
class ActorHasTagTest;
class ActorHasTargetTest;
class ActorInCaravanTest;
class ActorInCloudsTest;
class ActorInLavaTest;
class ActorInVillageTest;
class ActorInWaterOrRainTest;
class ActorInWaterTest;
class ActorInWeatherTest;
class ActorInfo;
class ActorInfoRegistry;
class ActorInteraction;
class ActorIsAvoidingMobsTest;
class ActorIsClimbingTest;
class ActorIsColorTest;
class ActorIsFamilyTest;
class ActorIsImmobileTest;
class ActorIsLeashedTest;
class ActorIsLeashedToTest;
class ActorIsMarkVariantTest;
class ActorIsMovingTest;
class ActorIsOwnerTest;
class ActorIsRidingTest;
class ActorIsSkinIDTest;
class ActorIsSleepingTest;
class ActorIsSneakingTest;
class ActorIsTargetTest;
class ActorIsVariantTest;
class ActorIsVisibleTest;
class ActorLink;
class ActorMapping;
class ActorOnGroundTest;
class ActorOnLadderTest;
class ActorParticleEffect;
class ActorParticleEffectEvent;
class ActorPickRequestPacket;
class ActorRiderCountTest;
class ActorRuntimeID;
class ActorServerCommandOrigin;
class ActorSkeletalAnimation;
class ActorSkeletalAnimationPlayer;
class ActorSkeletalAnimationPtr;
class ActorSoundEffect;
class ActorSoundEffectEvent;
class ActorSpawnRuleBase;
class ActorSpawnRuleGroup;
class ActorTerrainInterlockData;
class ActorTrustsSubjectTest;
class ActorUndergroundTest;
class ActorUnderwaterTest;
class ActorUniqueID;
class AddActorPacket;
class AddBehaviorTreePacket;
class AddEntityPacket;
class AddItemActorPacket;
class AddOceanTemperatureLayer;
class AddPaintingPacket;
class AddPlayerPacket;
class AddRiderComponent;
class AddRiderDefinition;
class AdventureSettings;
class AdventureSettingsPacket;
class AgeableComponent;
class AgeableDefinition;
class AgeableSystem;
class Agent;
class AgentBodyControl;
class AgentCommand;
class AgentCommandComponent;
class AgentCommandExecutionGoal;
class AgentCommandSystem;
namespace AgentCommands {
    class AttackCommand;
}
namespace AgentCommands {
    class CollectCommand;
}
namespace AgentCommands {
    class Command;
}
namespace AgentCommands {
    class DestroyCommand;
}
namespace AgentCommands {
    class DetectCommand;
}
namespace AgentCommands {
    class DetectRedstoneCommand;
}
namespace AgentCommands {
    class DropAllCommand;
}
namespace AgentCommands {
    class DropCommand;
}
namespace AgentCommands {
    class GetItemCountCommand;
}
namespace AgentCommands {
    class GetItemDetailsCommand;
}
namespace AgentCommands {
    class GetItemSpaceCommand;
}
namespace AgentCommands {
    class InspectCommand;
}
namespace AgentCommands {
    class InspectDataCommand;
}
namespace AgentCommands {
    class MoveCommand;
}
namespace AgentCommands {
    class PlaceCommand;
}
namespace AgentCommands {
    class TillCommand;
}
namespace AgentCommands {
    class TransferToCommand;
}
namespace AgentCommands {
    class TurnCommand;
}
class AgentLookControl;
class AgentRenderData;
class AirBlock;
class AirBlockItem;
class AllWorkerConfigurations;
class AmbientSoundIntervalDescription;
class AmphibiousMoveControl;
class AmphibiousMoveControlDescription;
class Amplifier;
class AngryComponent;
class AngryDescription;
class AngrySystem;
class Animal;
class AnimatePacket;
class AnimatedImageData;
class AnimationComponent;
class AnimationComponentID;
class AnimationResourceDefinitionMap;
class AnimationScriptsDescription;
class AnimationValueCurveKeyFrame;
class AnimationsDescription;
class AnvilBlock;
class AnvilContainerManagerModel;
class AnvilDamagePacket;
class AppConfigs;
class AppLifecycleContext;
class AppPlatform;
class AppPlatformListener;
class AppPlatform_linux;
class ApplyItemBinding;
class ApplyLegacyEntityBinding;
class ArbitraryBiomeComponent;
class AreaAttackComponent;
class AreaAttackDefinition;
class AreaAttackSystem;
class AreaEffectCloud;
class ArmorContainerController;
class ArmorItem;
class ArmorStand;
class ArmorStandItem;
class Arrow;
class ArrowEffectSubcomponent;
class ArrowItem;
class AsyncTracker;
class AsynchronousIPResolver;
class AtlasItemManager;
class AttackDamageMobEffect;
class AttackDefinition;
class AttackDescription;
class AttackNode;
class Attribute;
class AttributeBuff;
class AttributeBuffInfo;
class AttributeCollection;
class AttributeDescription;
class AttributeInstance;
class AttributeInstanceDelegate;
class AttributeInstanceHandle;
class AttributeModifier;
class AutoCompleteInformation;
class AutoCompleteOption;
namespace Automation {
    class AutomationClient;
}
namespace Automation {
    class MessageHeader;
}
namespace Automation {
    class Response;
}
class AutomationClientConnectPacket;
class AutomationPlayerCommandOrigin;
class AuxDataBlockItem;
class AvailableActorIdentifiersPacket;
class AvailableCommandsPacket;
class AvoidMobGoal;
class AvoidMobTypeGoal;
class BackgroundTask;
class BackgroundTaskQueue;
class BackgroundWorker;
class BackgroundWorkerPerfInfo;
class BackwardsCompatTextureGroup;
class BackwardsCompatTextureInfo;
class Balloon;
class BalloonComponent;
class BalloonDefinition;
class BalloonItem;
class BalloonSystem;
class BalloonableComponent;
class BalloonableDefinition;
class BambooBlock;
class BambooBlockItem;
class BambooFeature;
class BambooSapling;
class BannerAddPatternRecipe;
class BannerBlock;
class BannerBlockActor;
class BannerDuplicateRecipe;
class BannerItem;
class BannerPattern;
class BannerPatternItem;
class BarrelBlock;
class BarrelBlockActor;
class BarrierBlock;
class BaseAttributeMap;
class BaseCircuitComponent;
class BaseCommandBlock;
class BaseContainerMenu;
class BaseGamePackSlices;
class BaseGameVersion;
class BaseMobSpawner;
class BaseMoveToBlockGoal;
class BaseMoveToGoal;
class BasePressurePlateBlock;
class BaseRailBlock;
class BaseRailTransporter;
class BasicTimer;
class Bat;
class BatchedNetworkPeer;
class BeaconBeamSection;
class BeaconBlock;
class BeaconBlockActor;
class BeaconContainerManagerModel;
class BeaconPaymentContainerController;
class BeardKernel;
class BedBlock;
class BedBlockActor;
class BedHelper;
class BedItem;
namespace Bedrock {
    class LogEndPoint;
}
namespace Bedrock {
    namespace Threading {
        class AsyncBase;
    }
}
class BedrockBlock;
namespace BedrockEngine {
    class AppIsland;
}
namespace BedrockEngine {
    class CommonPlatform;
}
namespace BedrockEngine {
    class IIslandCore;
}
namespace BedrockEngine {
    class PlatformBuildInfo;
}
namespace BedrockEngine {
    class PlatformRuntimeInfo;
}
namespace BedrockLog {
    class CategoryLogFile;
}
namespace BedrockLog {
    class CategoryLogs;
}
namespace BedrockLog {
    class LogAreaFilter;
}
namespace BedrockLog {
    class LogDetails;
}
class Bee;
class BeehiveBlock;
class BeehiveBlockActor;
class BeetrootBlock;
class BegGoal;
class BehaviorComponent;
class BehaviorData;
class BehaviorDefinition;
class BehaviorFactory;
class BehaviorNode;
class BehaviorPackContents;
class BehaviorSystem;
class BehaviorTreeDefinition;
class BehaviorTreeDefinitionPtr;
class BehaviorTreeDescription;
class BehaviorTreeGroup;
class BellBlock;
class BellBlockActor;
class BellBlockItem;
class BigEndianStringByteInput;
class BinaryDataInput;
class BinaryDataOutput;
class BinaryHeap;
class BinaryStream;
class Biome;
class BiomeArea;
class BiomeChunkState;
class BiomeComponentFactory;
class BiomeDefinitionListPacket;
class BiomeFilterGroup;
class BiomeHeight;
class BiomeMetadata;
class BiomeRegistry;
class BiomeRegistryMergeStrategy;
class BiomeSource;
class BiomeSourceGetBiomeCache;
class BirchFeature;
class Blacklist;
class BlastFurnaceBlock;
class BlastFurnaceBlockActor;
class BlastFurnaceContainerManagerModel;
class Blaze;
class Block;
class BlockActor;
class BlockActorDataPacket;
class BlockActorLevelListener;
class BlockBlobFeature;
class BlockBreakSensorComponent;
class BlockBreakSensorDefinition;
class BlockBreakSensorSystem;
class BlockChange;
class BlockCommandOrigin;
class BlockComponentDescription;
class BlockComponentFactory;
class BlockDefinition;
class BlockDefinitionGroup;
class BlockDescription;
class BlockDestroyTimeDescription;
class BlockEventCoordinator;
class BlockEventDispatcher;
class BlockEventDispatcherToken;
class BlockEventListener;
class BlockEventPacket;
class BlockExplodeableDescription;
class BlockFetchResult;
class BlockFlammableDescription;
class BlockFrictionDescription;
namespace BlockGeometry {
    class Element;
}
namespace BlockGeometry {
    class ElementBox;
}
namespace BlockGeometry {
    class Face;
}
namespace BlockGeometry {
    class Model;
}
namespace BlockGeometry {
    class TessellatedModel;
}
class BlockGraphics;
class BlockID;
class BlockIntersectionConstraint;
class BlockIsNameTest;
class BlockItem;
class BlockLayer;
class BlockLegacy;
class BlockLightDescription;
class BlockLightEmissionDescription;
class BlockListEventMap;
class BlockMapColorDescription;
class BlockPalette;
class BlockPatternBuilder;
class BlockPickRequestPacket;
class BlockPileFeature;
class BlockPlanterItem;
class BlockPos;
class BlockPosIterator;
class BlockReducer;
class BlockSelector;
namespace BlockSerializationUtils {
    class NbtToBlockCache;
}
class BlockSet;
class BlockSource;
class BlockSourceListener;
class BlockTickingQueue;
class BlockVolume;
class BlueIceBlock;
class BlueIceFeature;
class Boat;
class BoatItem;
class BodyControl;
class BodyControlComponent;
class BodyControlSystem;
class BoneAnimation;
class BoneAnimationChannel;
class BoneOrientation;
class BoneOrientationTransform;
class BonusChestFeature;
class BookCloningRecipe;
class BookEditPacket;
class BookshelfBlock;
class BoolOption;
class BoostItem;
class BoostableComponent;
class BoostableDefinition;
class BoostableSystem;
class BossComponent;
class BossDefinition;
class BossEventPacket;
class BossSystem;
class BottleItem;
class BoundingBox;
class Bounds;
class BowEnchant;
class BowItem;
class BreakBlockDefinition;
class BreakBlockNode;
class BreakBlocksComponent;
class BreakBlocksDescription;
class BreakBlocksSystem;
class BreakDoorAnnotationComponent;
class BreakDoorAnnotationDescription;
class BreakDoorAnnotationSystem;
class BreakDoorGoal;
class BreathableComponent;
class BreathableDefinition;
class BreathableSystem;
class BreatheAirGoal;
class BreedGoal;
class BreedableComponent;
class BreedableDefinition;
class BreedableSystem;
class BreedableType;
class BrewingStandBlock;
class BrewingStandBlockActor;
class BrewingStandContainerManagerModel;
class BrewingStandFuelContainerController;
class BrewingStandInputContainerController;
class BrewingStandResultContainerController;
class BribeableComponent;
class BribeableDefinition;
class BribeableSystem;
class Brightness;
class BrightnessPair;
class BubbleColumnBlock;
class BucketItem;
class BucketableComponent;
class BucketableDescription;
class BuildMatch;
class BuriedTreasureFeature;
class BuriedTreasurePiece;
class BuriedTreasureStart;
class BurnsInDaylightSystem;
class BurstReactionComponent;
class BushBlock;
class ButtonBlock;
class ByteArrayTag;
class ByteTag;
class BytesDataInput;
class BytesDataOutput;
class CSHA1;
class CactusBlock;
class CactusFeature;
class CakeBlock;
class CameraBlock;
class CameraItem;
class CameraItemComponent;
class CameraPacket;
class CampfireBlock;
class CampfireBlockActor;
class CanClimbDescription;
class CanFlyDescription;
class CanPowerJumpDescription;
class CanyonFeature;
class CapacitorComponent;
class CarrotBlock;
class CarrotOnAStickItem;
class CartographyInputContainerController;
class CartographyTableBlock;
class Cat;
class CatalogPackManifest;
class CatchFireSubcomponent;
class CauldronBlock;
class CauldronBlockActor;
class CaveSpider;
class CentralSpikedFeature;
class Certificate;
class ChalkboardBlockActor;
class ChangeDimensionPacket;
class ChangeDimensionRequest;
class ChangeSettingCommand;
class ChannelTransform;
class ChargeAttackGoal;
class ChargeHeldItemGoal;
class ChemicalHeatBlock;
class ChemistryAuxDataBlockItem;
class ChemistryBlockItem;
class ChemistryIngredient;
class ChemistryItem;
class ChemistryStickItem;
class ChemistryTableBlock;
class ChemistryTableBlockActor;
class ChestBlock;
class ChestBlockActor;
class ChestContainerManagerModel;
class Chicken;
class ChorusFlowerBlock;
class ChorusPlantBlock;
class ChunkBlockPos;
class ChunkBuildOrderPolicy;
class ChunkBuildOrderPolicyBase;
class ChunkPos;
class ChunkRadiusUpdatedPacket;
class ChunkSource;
class ChunkViewSource;
class CircleAroundAnchorGoal;
class CircuitComponentList;
class CircuitSceneGraph;
class CircuitSystem;
class CircuitTrackingInfo;
class ClacksServer;
class ClassroomModeListener;
class ClassroomModeNetworkHandler;
class ClayBlock;
class ClayFeature;
class ClearCommand;
class ClientAutomationCommandOrigin;
namespace ClientBlobCache {
    namespace Server {
        class ActiveTransfer;
    }
}
namespace ClientBlobCache {
    namespace Server {
        class ActiveTransfersManager;
    }
}
namespace ClientBlobCache {
    namespace Server {
        class Blob;
    }
}
namespace ClientBlobCache {
    namespace Server {
        class TransferBuilder;
    }
}
class ClientCacheBlobStatusPacket;
class ClientCacheMissResponsePacket;
class ClientCacheStatusPacket;
class ClientLevelEventCoordinator;
class ClientPlayerEventCoordinator;
class ClientToServerHandshakePacket;
class ClientboundMapItemDataPacket;
class ClockItem;
class ClockSpriteCalculator;
class CloneCommand;
class ClothBlock;
class ClothBlockItem;
class CoalItem;
class CocoaBlock;
class CollisionBoxDescription;
class Color;
class Color2Description;
class ColorDescription;
class ColorPaletteAttributes;
class ColoredBlock;
class ColoredTorchBlock;
class Command;
class CommandArea;
class CommandAreaFactory;
class CommandBlock;
class CommandBlockActor;
class CommandBlockComponent;
class CommandBlockDescription;
class CommandBlockSystem;
class CommandBlockUpdatePacket;
class CommandContext;
class CommandDispatcher;
class CommandFilePath;
class CommandFlag;
class CommandFunctionEntry;
class CommandIntegerRange;
class CommandItem;
class CommandLexer;
class CommandMessage;
class CommandOrigin;
class CommandOriginData;
class CommandOutput;
class CommandOutputMessage;
class CommandOutputPacket;
class CommandOutputParameter;
class CommandOutputSender;
class CommandParameterData;
class CommandPosition;
class CommandPositionFloat;
class CommandPropertyBag;
class CommandRawText;
class CommandRegistry;
class CommandRequestPacket;
class CommandSelectorBase;
class CommandSoftEnumRegistry;
class CommandSyntaxInformation;
class CommandVersion;
class CommandWildcardInt;
class CompactionListenerEnv;
class ComparatorBlock;
class ComparatorBlockActor;
class ComparatorCapacitor;
class CompassItem;
class CompassSpriteCalculator;
class CompletedUsingItemPacket;
class ComplexInventoryTransaction;
class ComplexItem;
class ComponentDescription;
class CompositeDefinition;
class CompositePackSource;
class ComposterBlock;
class CompoundCreatorContainerManagerModel;
class CompoundCreatorInputContainerController;
class CompoundItem;
class CompoundTag;
class CompoundTagEditHelper;
class CompoundTagUpdater;
class CompoundTagUpdaterBuilder;
class CompoundTagUpdaterContext;
class CompoundTagVariant;
class CompressedNetworkPeer;
class ConcreteBlock;
class ConcretePowderBlock;
class ConduitBlock;
class ConduitBlockActor;
class ConnectionDefinition;
class ConnectionRequest;
class Connector;
class ConsoleChunkBlender;
class ConsoleInputReader;
class ConsumeItemDefinition;
class ConsumeItemNode;
class ConsumerComponent;
class Container;
class ContainerClosePacket;
class ContainerComponent;
class ContainerContentChangeListener;
class ContainerController;
class ContainerDescription;
class ContainerItemStack;
class ContainerManagerModel;
class ContainerModel;
class ContainerOpenPacket;
class ContainerSetDataPacket;
class ContainerSizeChangeListener;
class ContentIdentity;
class ContentLog;
class ContentLogEndPoint;
class ContentLogFileEndPoint;
class ContentTierInfo;
class ContentTierManager;
class ContextAccessor;
class ContextMessage;
class ContextMessageLogger;
class ContextMessageLoggerOptions;
class Control;
class ControlledByPlayerGoal;
class Coral;
class CoralBlock;
class CoralCrustFeature;
class CoralFan;
class CoralFanBlockItem;
class CoralFanHang;
class CoralFeature;
class CoralHangFeature;
namespace Core {
    class DirectoryIterationItem;
}
namespace Core {
    class DiskAccessDiagnostics;
}
namespace Core {
    class DiskAccessTracker;
}
namespace Core {
    class ExcludedPath;
}
namespace Core {
    class File;
}
namespace Core {
    class FileImpl;
}
namespace Core {
    class FileOpenMode;
}
namespace Core {
    class FilePathManager;
}
namespace Core {
    class FileStats;
}
namespace Core {
    class FileStdStreamBuf;
}
namespace Core {
    class FileStorageArea;
}
namespace Core {
    class FileStream;
}
namespace Core {
    namespace FileSystem {
        class BasicFileData;
    }
}
namespace Core {
    namespace FileSystem {
        class FileTransferProgress;
    }
}
namespace Core {
    class FileSystemImpl;
}
namespace Core {
    class FileSystem_generic;
}
namespace Core {
    class File_c;
}
namespace Core {
    class FlatFile;
}
namespace Core {
    class FlatFileManifest;
}
namespace Core {
    class FlatFileManifestInfo;
}
namespace Core {
    class FlatFileManifestTracker;
}
namespace Core {
    class FlatFileSearchResult;
}
namespace Core {
    class FlatFileSystemImpl;
}
namespace Core {
    class InputFileStream;
}
namespace Core {
    class LevelStorageResult;
}
namespace Core {
    class LoadTimeData;
}
namespace Core {
    class LoadTimeProfiler;
}
namespace Core {
    class OutputFileStream;
}
namespace Core {
    class Path;
}
namespace Core {
    class PathPart;
}
namespace Core {
    namespace Profile {
        class CPUProfileToken;
    }
}
namespace Core {
    namespace Profile {
        class CounterToken;
    }
}
namespace Core {
    namespace Profile {
        class ProfileMultiSectionCPU;
    }
}
namespace Core {
    namespace Profile {
        class ProfileSectionCPU;
    }
}
namespace Core {
    namespace Profile {
        class ProfileThread;
    }
}
namespace Core {
    class Random;
}
namespace Core {
    class Result;
}
namespace Core {
    class ScopedLoadTimeSection;
}
namespace Core {
    class SingleThreadedLock;
}
namespace Core {
    class StorageAreaState;
}
namespace Core {
    class StorageAreasTree;
}
namespace Core {
    class StringSpan;
}
namespace Core {
    class TransactionFrame;
}
namespace Core {
    class TransactionFrameSourceTarget;
}
namespace Core {
    class UnzipFile;
}
namespace Core {
    class UnzipInternals;
}
namespace Core {
    namespace ZipUtils {
        class UnzipSettings;
    }
}
namespace Core {
    namespace ZipUtils {
        class ZipFileRestrictions;
    }
}
namespace Core {
    namespace ZipUtils {
        class ZipProgress;
    }
}
namespace Core {
    namespace ZipUtils {
        class ZipProgressList;
    }
}
namespace Core {
    namespace ZipUtils {
        class ZipSettings;
    }
}
class CraftableCompounds;
class CraftingContainer;
class CraftingContainerController;
class CraftingDataEntry;
class CraftingDataPacket;
class CraftingEventPacket;
class CraftingInputContainerController;
class CraftingOutputContainerController;
class CreativeContainerController;
class CreativeGroupInfo;
class Creeper;
class CropBlock;
class CrossbowEnchant;
class CrossbowItem;
namespace Crypto {
    namespace Asymmetric {
        class Asymmetric;
    }
}
namespace Crypto {
    namespace Hash {
        class HMAC;
    }
}
namespace Crypto {
    namespace Hash {
        class Hash;
    }
}
namespace Crypto {
    namespace Random {
        class Random;
    }
}
namespace Crypto {
    namespace Symmetric {
        class Symmetric;
    }
}
class CustomScriptEventData;
class CustomSkyColorAttributes;
class DBChunkStorage;
class DBChunkStorageKey;
class DBStorage;
class DBStorageConfig;
class DBStorageEnvironmentChain;
class DBStorageWriteBatch;
class DamageCondition;
class DamageOverTimeComponent;
class DamageOverTimeDefinition;
class DamageOverTimeSystem;
class DamageSensorComponent;
class DamageSensorDefinition;
class DamageSensorTrigger;
class DanceComponent;
class DanceComponentListener;
class DanceDescription;
class DanceSystem;
class DataItem;
class DataLoadHelper;
namespace DataStructures {
    class ByteQueue;
}
class DatagramHeaderFormat;
class DateManager;
class DayLockCommand;
class DaylightDetectorBlock;
class DaylightDetectorBlockActor;
class DeOpCommand;
class DeadBush;
class DeadBushFeature;
class DebugAssertException;
class DebugLogScope;
class DecoratorDefinition;
class DedicatedServer;
class DedicatedWSServerCommand;
class DefaultDataLoadHelper;
class DefaultEmptyActorAnimationPlayer;
class DefaultLookAngleDescription;
class DefaultMobSpawner;
class DefendTrustedTargetGoal;
class DefendVillageTargetGoal;
class DefinitionEvent;
class DefinitionInstanceGroup;
class DefinitionModifier;
class DefinitionTrigger;
class DefintionDescription;
class DelayedAttackGoal;
class DenySameParentsVariantData;
class Description;
class DesertPyramidPiece;
class DesertWellFeature;
class DespawnComponent;
class DespawnDescription;
class DespawnSystem;
class DetectorRailBlock;
class DevConsoleCommandOrigin;
class DifficultyCommand;
class DiggerItem;
class DiggingEnchant;
class Dimension;
class DimensionConversionData;
class DiodeBlock;
class DirectoryPackAccessStrategy;
class DirectoryPackSource;
class DirectoryPackWithEncryptionAccessStrategy;
class DirtBlock;
class DirtyTicksCounter;
class DisconnectPacket;
class DispenserBlock;
class DispenserBlockActor;
class DispenserContainerManagerModel;
class DisplayObjective;
class DistanceConstraint;
class Dolphin;
class DolphinMoveControl;
class DoorBlock;
class DoorInteractGoal;
class DoorItem;
class DoublePlantBlock;
class DoublePlantFeature;
class DoubleTag;
class DouseFireSubcomponent;
class DragonChargePlayerGoal;
class DragonDeathGoal;
class DragonEggBlock;
class DragonFireball;
class DragonFlamingGoal;
class DragonHoldingPatternGoal;
class DragonLandingGoal;
class DragonScanningGoal;
class DragonStrafePlayerGoal;
class DragonTakeoffGoal;
class DriedKelpBlock;
class DrinkPotionData;
class DrinkPotionGoal;
class DropItemForGoal;
class DropperBlock;
class DropperBlockActor;
class DropperContainerManagerModel;
class DwellerComponent;
class DwellerDescription;
class DwellerSystem;
class DyePowderItem;
class DyeableDescription;
class DynamicJumpControl;
class DynamicJumpControlDescription;
class EatBlockGoal;
class EatCarriedItemGoal;
class EconomyTradeableComponent;
class EconomyTradeableDescription;
class EducationLevelSettings;
class EducationLocalLevelSettings;
class EducationMetadata;
class EducationMetadataError;
class EducationOptions;
class EducationSettingsPacket;
class EffectCommand;
class EggItem;
class ElementBlock;
class ElementBlockItem;
class ElementConstructorContainerManagerModel;
class EmotePacket;
class EmptyLootItem;
class EmptyMapItem;
class Enchant;
class EnchantBookForTradingFunction;
class EnchantCommand;
class EnchantRandomEquipmentFunction;
class EnchantRandomlyFunction;
class EnchantWithLevelsFunction;
class EnchantedBookItem;
class EnchantingContainerManagerModel;
class EnchantingInputContainerController;
class EnchantingMaterialContainerController;
class EnchantingTableBlock;
class EnchantingTableBlockActor;
class EnchantmentInstance;
class EncryptedFileAccessStrategy;
class EncryptedNetworkPeer;
class EncryptedProxyEnv;
class EncryptedZipTransforms;
class EndCityFeature;
namespace EndCityPieces {
    class EndCityPiece;
}
namespace EndCityPieces {
    class FatTowerGenerator;
}
namespace EndCityPieces {
    class HouseTowerGenerator;
}
namespace EndCityPieces {
    class SectionGenerator;
}
namespace EndCityPieces {
    class TowerBridgeGenerator;
}
namespace EndCityPieces {
    class TowerGenerator;
}
class EndCityStart;
class EndCrystalItem;
class EndDragonFight;
class EndGatewayBlock;
class EndGatewayBlockActor;
class EndGatewayFeature;
class EndIslandFeature;
class EndPodiumFeature;
class EndPortalBlock;
class EndPortalBlockActor;
class EndPortalFrameBlock;
class EndPortalShape;
class EndRodBlock;
class EndTag;
class EnderChestBlock;
class EnderChestBlockActor;
class EnderChestContainer;
class EnderCrystal;
class EnderDragon;
class EnderEyeItem;
class EnderMan;
class EndermanLeaveBlockGoal;
class EndermanTakeBlockGoal;
class Endermite;
class EnderpearlItem;
class EntityClientPacket;
class EntityComponentFactory;
class EntityContext;
class EntityContextBase;
class EntityId;
class EntityNetId;
class EntityOptionalOwnerRef;
class EntityRegistry;
class EntityRegistryBase;
class EntityRegistryOwned;
class EntitySensorComponent;
class EntitySensorDefinition;
class EntitySensorSystem;
class EntityServerPacket;
class EntitySystems;
class EnumOption;
class EnvironmentRequirement;
class EnvironmentSensorDefinition;
class EnvironmentSensorSystem;
class EquipmentTableDescription;
class EquippableComponent;
class EquippableDefinition;
class ErrorPathStack;
class EvalParams;
class EventInfo;
class EventPacket;
class EvocationFang;
class EvocationIllager;
class ExecCtxNext;
class ExecCtxPluck;
class ExecuteCommand;
class ExhaustionAttributeDelegate;
class ExpandoContainerModel;
class ExpandoModelElement;
class ExperienceOrb;
class ExperiencePotion;
class ExperiencePotionItem;
class ExperienceRewardComponent;
class ExperienceRewardDefinition;
class ExplodeComponent;
class ExplodeDefinition;
class ExplodeSystem;
class ExplorationMapFunction;
class ExploreOutskirtsGoal;
class Explosion;
class ExplosionDecayFunction;
class ExpressionNode;
class ExternalFileLevelStorageSource;
class EyeOfEnder;
class FaceDirectionalActorBlock;
class FaceDirectionalBlock;
class FallingBlock;
class FamilyTypeDescription;
class FancyTreeFeature;
class FarmBlock;
class Feature;
namespace FeatureLoading {
    class FeatureRootParseContext;
}
class FeaturePoolElement;
class FeatureRegistry;
class FeatureToggles;
class FeatureTypeFactory;
class FeedItem;
class FenceBlock;
class FenceGateBlock;
class FertilizerItem;
class FileAccessTransforms;
class FileArchiver;
class FileChunk;
class FileChunkInfo;
class FileChunkManager;
class FileInfo;
class FilePickerSettings;
class FileSystemFileAccess;
class FileUploadManager;
class FillCommand;
class FillContainerFunction;
class FillingContainer;
class FilterContext;
class FilterGroup;
class FilterInput;
class FilterInputDefinition;
class FilterInputs;
class FilterParamDefinition;
class FilterStringMap;
class FilterTest;
class FilterTestAltitude;
class FilterTestBiome;
class FilterTestBiomeHasTag;
class FilterTestBiomeHumid;
class FilterTestBiomeSnowCovered;
class FilterTestBrightness;
class FilterTestClock;
class FilterTestDaytime;
class FilterTestDifficulty;
class FilterTestDistanceToNearestPlayer;
class FilterTestGameRule;
class FilterTestHasTradeSupply;
class FilterTestHourlyClock;
class FilterTestMoonIntensity;
class FilterTestMoonPhase;
class FilterTestTemperatureType;
class FilterTestTemperatureValue;
class FilteredContainerModel;
class FindActorDefinition;
class FindActorNode;
class FindBlockDefinition;
class FindBlockNode;
class FindCoverGoal;
class FindMountGoal;
class FindUnderwaterTreasureGoal;
class FireBlock;
class FireChargeItem;
class FireImmuneDescription;
class Fireball;
class FireworkChargeItem;
class FireworksItem;
class FireworksRecipe;
class FireworksRocketActor;
class Fish;
class FishingEnchant;
class FishingHook;
class FishingRodItem;
class FitDoubleXRoom;
class FitDoubleXYRoom;
class FitDoubleYRoom;
class FitDoubleYZRoom;
class FitDoubleZRoom;
class FitSimpleRoom;
class FitSimpleTopRoom;
class FixedBiomeSource;
class FlatWorldGenerator;
class FlatWorldGeneratorOptions;
class FleeSunGoal;
class FlintAndSteelItem;
class FloatGoal;
class FloatNavigation;
class FloatOption;
class FloatRange;
class FloatTag;
class FloatWanderGoal;
class FloatsInLiquidDescription;
class FlockingComponent;
class FlockingDefinition;
class FlockingSystem;
class FlowerBlock;
class FlowerFeature;
class FlowerPotBlock;
class FlowerPotBlockActor;
class FlushableEnv;
class FlushableStorageAreaEnv;
class FlyDefinition;
class FlyMoveControl;
class FlyNode;
class FlyingPathNavigation;
class FlyingSpeedDescription;
class FollowCaravanGoal;
class FollowFlockGoal;
class FollowMobGoal;
class FollowOwnerGoal;
class FollowParentGoal;
class FollowTargetCaptainGoal;
class FoodItemComponent;
class FootSizeDescription;
class FossilFeature;
class FreezeOnHitSubcomponent;
class FrictionModifierDescription;
class FriendlySize;
class FrostWalkerEnchant;
class FrostedIceBlock;
class FullPlayerInventoryWrapper;
class FunctionCommand;
class FunctionEntry;
class FunctionManager;
class FurnaceBlock;
class FurnaceBlockActor;
class FurnaceContainerManagerModel;
class FurnaceFuelContainerController;
class FurnaceInputContainerController;
class FurnaceResultContainerController;
class GameArgumentCommandOrigin;
class GameCallbacks;
class GameMasterEntityServerCommandOrigin;
class GameMode;
class GameModeCommand;
class GameModuleServer;
class GameRule;
class GameRuleCommand;
class GameRuleId;
class GameRules;
class GameRulesChangedPacket;
class GameRulesChangedPacketData;
class GameSession;
class GameVersion;
class GeneDefinition;
class GenericMoveControl;
class GenericMoveControlDescription;
class GenericPathNavigation;
class GeneticVariant;
class GeneticsComponent;
class GeneticsDefinition;
class GetChunkDataCommand;
class GetChunksCommand;
class GetEduServerInfoCommand;
class GetInteractionPositionForBlockDefinition;
class GetInteractionPositionForBlockNode;
class GetSpawnPointCommand;
class GetTopSolidBlockCommand;
class Ghast;
class GiveCommand;
class GiveableComponent;
class GiveableDefinition;
class GiveableTrigger;
class GlassBlock;
class GlazedTerracottaBlock;
class GlideMoveControl;
class GlideMoveControlDescription;
class GlobalPauseCommand;
class GlowStickItem;
class GlowStoneFeature;
class GoHomeGoal;
class Goal;
class GoalDefinition;
class GoalSelectorComponent;
class GoalSelectorSystem;
class GrantXPSubcomponent;
class GrassBlock;
class GrassPathBlock;
class GravelBlock;
class GrindstoneBlock;
class GrindstoneContainerController;
class GrindstoneContainerManagerModel;
class GroundBushFeature;
class GroundOffsetDescription;
class GroundedConstraint;
class GrowCropSystem;
class GrowsCropComponent;
class GrowsCropDefinition;
class Guardian;
class GuardianAttackGoal;
class GuiDataPickItemPacket;
class HangingActor;
class HangingActorItem;
class HardcodedSpawnAreaRegistry;
class HarvestFarmBlockGoal;
class HashedString;
class HatchetItem;
class HayBlockBlock;
class HealableComponent;
class HealableDefinition;
class HealthAttributeDelegate;
class HeavyBlock;
class HellFireFeature;
class HellSpringFeature;
class HelpCommand;
class HideComponent;
class HideDescription;
class HideGoal;
class HitResult;
class HoeItem;
class HoldGroundGoal;
class HomeComponent;
class HoneyBlock;
class HoneycombBlock;
class HopMoveControl;
class Hopper;
class HopperBlock;
class HopperBlockActor;
class HopperComponent;
class HopperContainerManagerModel;
class HopperSystem;
class Horse;
class HorseArmorItem;
class HorseContainerManagerModel;
class HorseEquipContainerController;
class HoverGoal;
class HoverMoveControl;
class HoverPathNavigation;
class HudContainerManagerModel;
class HudContainerModel;
class HugeMushroomBlock;
class HugeMushroomFeature;
class HumanoidMonster;
class HungerAttributeDelegate;
class HurtArmorPacket;
class HurtByTargetGoal;
class HurtOnConditionComponent;
class HurtOnConditionDefinition;
class HurtOnConditionSystem;
class HurtOwnerSubcomponent;
namespace I18n {
    class LanguageChangedListener;
}
class IAppPlatform;
class IBlockPlacementTarget;
class ICommandDispatcher;
class IContainerManager;
class IContentAccessibilityProvider;
class IContentKeyProvider;
class IDataInput;
class IDataOutput;
class IDefinitionInstance;
class IDefinitionSerializer;
class IEntityComponent;
class IEntityRegistryOwner;
class IFeature;
class IFileAccess;
class IFileChunkUploader;
class IFilePicker;
class IFileReadAccess;
class IFileWriteAccess;
class IFunctionEntry;
class IGameModuleShared;
class IInPackagePacks;
class IJsonSerializable;
class IMinecraftApp;
class IMinecraftEventing;
class IPackTelemetry;
class IPacketHandlerDispatcher;
class IStructureConstraint;
class IStructurePoolActorPredicate;
class IStructurePoolBlockPredicate;
class IStructurePoolBlockTagPredicate;
class ISubChunkBlockStoragePaletted;
class ISurfaceBuilder;
class ITaskExecutionContext;
class ITaskGroup;
class ITextObject;
class ITickingArea;
class ITickingAreaView;
class ITickingSystem;
class IWebviewTelemetry;
class IWorldRegistriesProvider;
class IceBlock;
class IceBombItem;
class IcePatchFeature;
class IceSpikeFeature;
class IcebergFeature;
class IdentifierDescription;
class IdentifierResult;
class IdentityDefinition;
class IdentityDictionary;
class Igloo;
class IgniteSubcomponent;
class IllagerBeast;
class IllagerBeastBlockedComponent;
class IllagerBeastBlockedDescription;
class ImmutableWorldCommand;
class ImpactDamageSubcomponent;
class ImprovedNoise;
class InMemoryEnv;
class InMemoryFile;
class InMemoryFileStorage;
class InMemoryRandomAccessFile;
class InMemorySequentialFile;
class InMemoryWritableFile;
class InPackagePackSource;
class IndexSet;
class InputModeBoolOption;
class InputModeFloatOption;
class InsomniaComponent;
class InsomniaDefinition;
class InsomniaSystem;
class InspectBookshelfGoal;
class InstantaneousAttributeBuff;
class InstantaneousMobEffect;
class Int64Tag;
class IntArrayTag;
class IntOption;
class IntRange;
class IntTag;
class InteractActionDefinition;
class InteractActionNode;
class InteractComponent;
class InteractDefinition;
class InteractPacket;
class InteractSystem;
class Interaction;
class InternalTaskGroup;
class InvalidPacksFilterGroup;
class Inventory;
class InventoryAction;
class InventoryContainerModel;
class InventoryContentPacket;
class InventoryMenu;
class InventorySlotPacket;
class InventorySource;
class InventoryTransaction;
class InventoryTransactionItemGroup;
class InventoryTransactionManager;
class InventoryTransactionPacket;
class InverterDefinition;
class InverterNode;
class InvisibleBlock;
class IronGolem;
class IsBabyDescription;
class IsChargedDescription;
class IsChestedDescription;
class IsExperimentalDescription;
class IsHiddenWhenInvisibleDescription;
class IsIgnitedDescription;
class IsIllagerCaptainDescription;
class IsSaddledDescription;
class IsShakingDescription;
class IsShearedDescription;
class IsSpawnableDescription;
class IsStackableDescription;
class IsStunnedDescription;
class IsSummonableDescription;
class IsTamedDescription;
class IslandLayer;
class IsotropicFaceData;
class Item;
class ItemActor;
class ItemControlDescription;
class ItemData;
class ItemDefinition;
class ItemDescriptor;
class ItemDescriptorCount;
class ItemEnchants;
class ItemEventCoordinator;
class ItemFrameBlock;
class ItemFrameBlockActor;
class ItemFrameDropItemPacket;
class ItemGroup;
class ItemIndexPair;
class ItemInstance;
class ItemOffsetsJsonMergeStrategy;
class ItemPack;
class ItemReactionComponent;
class ItemReleaseInventoryTransaction;
class ItemStack;
class ItemStackBase;
class ItemState;
class ItemStateInstance;
class ItemTransferAmount;
class ItemUseInventoryTransaction;
class ItemUseOnActorInventoryTransaction;
class JigsawBlock;
class JigsawBlockActor;
class JigsawBlockInfo;
class JigsawJunction;
class JigsawStructureActorRulesRegistry;
class JigsawStructureBlockRulesRegistry;
class JigsawStructureBlockTagRulesRegistry;
class JigsawStructureElementRegistry;
class JigsawStructureRegistry;
class JournaledFile;
namespace Json {
    class FastWriter;
}
namespace Json {
    class Features;
}
namespace Json {
    class Path;
}
namespace Json {
    class PathArgument;
}
namespace Json {
    class Reader;
}
namespace Json {
    class StaticString;
}
namespace Json {
    class StyledStreamWriter;
}
namespace Json {
    class StyledWriter;
}
namespace Json {
    class Value;
}
namespace Json {
    class ValueConstIterator;
}
namespace Json {
    class ValueIterator;
}
namespace Json {
    class ValueIteratorBase;
}
namespace Json {
    class Writer;
}
class JsonMergeStrategy;
namespace JsonValidator {
    class Property;
}
class JukeboxBlock;
class JukeboxBlockActor;
class JumpControl;
class JumpControlComponent;
class JumpControlDescription;
class JumpControlSystem;
class JumpInfo;
class JunglePyramidPiece;
class JungleTreeFeature;
class KelpBlock;
class KelpFeature;
class KeyFrameLerpMode;
class KeyFrameTransform;
class KeyFrameTransformData;
class KeyManager;
class KickCommand;
class KillCommand;
class KnockbackRoarGoal;
class LabTableContainerManagerModel;
class LabTableInputContainerController;
class LabTablePacket;
class LabTableReaction;
class LabTableReactionComponent;
class LadderBlock;
class LakeFeature;
class LanternBlock;
class LargeCaveFeature;
class LargeFireball;
class LavaSlime;
class LayDownGoal;
class LayEggGoal;
class LayerBiomeSource;
namespace LayerDetails {
    class LayerBase;
}
namespace LayerDetails {
    class Storage;
}
namespace LayerFilters {
    class AddBiomeIsland;
}
namespace LayerFilters {
    class AddMushroomIsland;
}
namespace LayerFilters {
    class BiomeInit;
}
namespace LayerFilters {
    class PromoteCenter;
}
namespace LayerFilters {
    class RareBiomeSpot;
}
namespace LayerFilters {
    class RiverInit;
}
namespace LayerFilters {
    class Shore;
}
namespace LayerValues {
    class PreBiome;
}
class LeadItem;
class LeafBlock;
class LeafBlockItem;
class LeapAtTargetGoal;
class LeashFenceKnotActor;
class LeashableComponent;
class LeashableDefinition;
class LeashableSystem;
class LecternBlock;
class LecternBlockActor;
class LecternUpdatePacket;
class LegacyBlockPlacementProcessor;
class LegacyBodyControl;
class LegacyChunkStorage;
class LegacyEmeraldOreFeature;
class LegacyFlowerFeature;
class LegacyForestFoliageFeature;
class LegacyForestRockFeature;
class LegacyIceFeature;
class LegacyPackIdVersion;
class LegacySmallMushroomsFeature;
class LegacySpringsFeature;
class LegacyStructureActorInfo;
class LegacyStructureBlockInfo;
class LegacyStructureBlockPalette;
class LegacyStructureSettings;
class LegacyStructureTemplate;
class LegacySwampFoliageFeature;
class LegacyTradeableComponent;
class LegacyTradeableDefinition;
class LegacyTreeFeature;
class Level;
class LevelChunk;
class LevelChunkBlockActorAccessToken;
class LevelChunkBuilderData;
class LevelChunkGarbageCollector;
class LevelChunkPacket;
class LevelContainerManagerModel;
class LevelContainerModel;
class LevelData;
class LevelDataWrapper;
class LevelDbEnv;
class LevelDbFileLock;
class LevelDbLogger;
class LevelDbRandomAccessFile;
class LevelDbSequentialFile;
class LevelDbWritableFile;
class LevelEventCoordinator;
class LevelEventGenericPacket;
class LevelEventListener;
class LevelEventPacket;
class LevelListener;
class LevelSettings;
class LevelSoundEventPacket;
class LevelSoundEventPacketV1;
class LevelSoundEventPacketV2;
class LevelStorage;
class LevelStorageObserver;
class LevelStorageSource;
class LevelSummary;
class LeverBlock;
class LightBlock;
class LightGemBlock;
class LightningBolt;
class LingeringPotionItem;
class LiquidBlock;
class LiquidBlockDynamic;
class LiquidBlockStatic;
class ListCommand;
class ListDCommand;
class ListTag;
class ListTagFloatAdder;
class ListTagIntAdder;
class ListenerInfo;
class Llama;
class LlamaSpit;
class LoadedResourceData;
class LocalConnector;
class LocalNetworkPeer;
class Localization;
class LocateCommand;
class LogBlock;
class LoginPacket;
class LookAtActorDefinition;
class LookAtActorGoal;
class LookAtActorNode;
class LookAtBlockDefinition;
class LookAtBlockNode;
class LookAtComponent;
class LookAtDefinition;
class LookAtSystem;
class LookAtTradingPlayerGoal;
class LookControl;
class LookControlComponent;
class LookControlSystem;
class LoomBannerContainerController;
class LoomBlock;
class LoomDyeContainerController;
class LoomMaterialContainerController;
class LoopbackPacketSender;
class LootComponent;
class LootComponentDescription;
class LootEnchant;
class LootItem;
class LootItemActorHasMarkVariantCondition;
class LootItemCondition;
class LootItemFunction;
class LootItemKilledByActorCondition;
class LootItemKilledByPlayerCondition;
class LootItemKilledByPlayerOrPetsCondition;
class LootItemRandomChanceCondition;
class LootItemRandomChanceWithLootingCondition;
class LootItemRandomChanceWithSpecialModifierCondition;
class LootItemRandomDifficultyChanceCondition;
class LootPool;
class LootPoolEntry;
class LootTable;
class LootTableContext;
class LootTableDescription;
class LootTableEntry;
class LootTableReference;
class LootTables;
class LootingEnchantFunction;
class MCRESULT;
class MagmaBlock;
class MainChunkSource;
class MakeLoveGoal;
class ManagedWanderingTraderDescription;
class MapCloningRecipe;
class MapCreateLockedCopyPacket;
class MapDecoration;
class MapExtendingRecipe;
class MapInfoRequestPacket;
class MapItem;
class MapItemSavedData;
class MapItemTrackedActor;
class MapLockingRecipe;
class MapSample;
class MapUpgradingRecipe;
class MarkVariantDescription;
class Material;
class MaterialReducerContainerManagerModel;
class MaterialReducerInputContainerController;
class MaterialReducerOutputContainerController;
class MaterialVariants;
class Matrix;
class MeCommand;
class MedicineItem;
class MegaJungleTreeFeature;
class MegaPineTreeFeature;
class MegaTreeFeature;
class MeleeAttackGoal;
class MeleeWeaponEnchant;
class MelonBlock;
class MelonFeature;
class MemoryMappedFileAccess;
class MendingEnchant;
class MerchantRecipe;
class MerchantRecipeList;
class MesaSurfaceAttributes;
class MessagingCommand;
class MetalBlock;
class Minecart;
class MinecartBlockCommandOrigin;
class MinecartChest;
class MinecartCommandBlock;
class MinecartHopper;
class MinecartItem;
class MinecartRideable;
class MinecartTNT;
class Minecraft;
class MinecraftCommands;
class MinecraftEventing;
class MinecraftServerScriptEngine;
class MineshaftCorridor;
class MineshaftCrossing;
class MineshaftData;
class MineshaftFeature;
class MineshaftPiece;
class MineshaftRoom;
class MineshaftStairs;
class MineshaftStart;
class MingleComponent;
class MingleGoal;
class Mob;
class MobArmorEquipmentPacket;
class MobDescriptor;
class MobEffect;
class MobEffectChangeDescription;
class MobEffectComponent;
class MobEffectDefinition;
class MobEffectInstance;
class MobEffectPacket;
class MobEffectSubcomponent;
class MobEffectSystem;
class MobEquipmentPacket;
class MobEvent;
class MobEventCommand;
class MobEvents;
class MobPlacerItem;
class MobSpawnHerdInfo;
class MobSpawnRules;
class MobSpawnerBlock;
class MobSpawnerBlockActor;
class MobSpawnerData;
class MobSpawnerPermutation;
class ModalFormRequestPacket;
class ModalFormResponsePacket;
class ModelPartLocator;
class MolangArrayVariable;
class MolangDataDrivenGeometry;
class MolangEntityVariable;
class MolangGenericQueryFunction;
class MolangGenericQueryFunctionPtr;
class MolangGeometryVariable;
class MolangMaterialVariable;
class MolangQueryFunction;
class MolangQueryFunctionPtr;
class MolangScriptArg;
class MolangTempVariable;
class MolangTextureVariable;
class MolangVariable;
class MolangVariableMap;
class Monster;
class MonsterEggBlock;
class MonsterRoomFeature;
class MonumentBuilding;
class MonumentRoomFitter;
class MossStoneSelector;
class Motive;
class MountPathingGoal;
class MountTameableDefinition;
class MountTamingComponent;
class MountTamingSystem;
class MoveActorAbsoluteData;
class MoveActorAbsolutePacket;
class MoveActorDeltaData;
class MoveActorDeltaPacket;
class MoveControl;
class MoveControlBasicDescription;
class MoveControlComponent;
class MoveControlDescription;
class MoveControlDolphinDescription;
class MoveControlFlyDescription;
class MoveControlHoverDescription;
class MoveControlSkipDescription;
class MoveControlSwayDescription;
class MoveControlSystem;
class MoveDefinition;
class MoveIndoorsGoal;
class MoveInput;
class MoveInputHandler;
class MoveNode;
class MovePlayerPacket;
class MoveThroughVillageGoal;
class MoveToBlockGoal;
class MoveToDefinition;
class MoveToLandGoal;
class MoveToNode;
class MoveToPOIGoal;
class MoveToRandomBlockGoal;
class MoveToVillageGoal;
class MoveToWaterGoal;
class MoveTowardsRestrictionGoal;
class MoveTowardsTargetGoal;
class MovementInterpolator;
class MovingBlock;
class MovingBlockActor;
class MultiRecipe;
class MultiplayerSettingsPacket;
class MultiplyValueAmplifier;
class MushroomBlock;
class MushroomCow;
class MusicBlockActor;
class MutationFactorData;
class MyceliumBlock;
class NBBridgeCrossing;
class NBBridgeEndFiller;
class NBBridgeStraight;
class NBCastleCorridorStairsPiece;
class NBCastleCorridorTBalconyPiece;
class NBCastleEntrance;
class NBCastleSmallCorridorCrossingPiece;
class NBCastleSmallCorridorLeftTurnPiece;
class NBCastleSmallCorridorPiece;
class NBCastleSmallCorridorRightTurnPiece;
class NBCastleStalkRoom;
class NBMonsterThrone;
class NBRoomCrossing;
class NBStairsRoom;
class NBStartPiece;
class NameAction;
class NameableComponent;
class NameableDescription;
class NapGoal;
class NavigationClimbDescription;
class NavigationComponent;
class NavigationDescription;
class NavigationFloatDescription;
class NavigationFlyDescription;
class NavigationGenericDescription;
class NavigationHoverDescription;
class NavigationSwimDescription;
class NavigationSystem;
class NavigationWalkDescription;
class NearestAttackableTargetGoal;
class NearestPrioritizedAttackableTargetGoal;
class NetEventCallback;
class NetherDimension;
class NetherFortressFeature;
class NetherFortressPiece;
class NetherFortressStart;
class NetherGenerator;
class NetherReactorBlock;
class NetherReactorBlockActor;
class NetherWartBlock;
class NetworkAddress;
class NetworkBlockPosition;
class NetworkChunkPublisher;
class NetworkChunkPublisherUpdatePacket;
class NetworkChunkSource;
class NetworkComponent;
class NetworkDebugManager;
class NetworkHandler;
class NetworkIdentifier;
class NetworkPacketEventCoordinator;
class NetworkPacketEventListener;
class NetworkPeer;
class NetworkSettingsPacket;
class NetworkStackLatencyPacket;
class NetworkStatistics;
class NewBlockID;
class NewLeafBlock;
class NewLogBlock;
class NoteBlock;
class Npc;
class NpcAction;
class NpcCommandAction;
class NpcComponent;
class NpcDescription;
class NpcGUIOffset;
class NpcRequestPacket;
class NpcUrlAction;
class NullLogger;
class NullSecureStorage;
class NullSoundPlayer;
class OakFeature;
class Objective;
class ObjectiveCriteria;
class ObserverBlock;
class ObsidianBlock;
class OceanMixerLayer;
class OceanMonumentCoreRoom;
class OceanMonumentDoubleXRoom;
class OceanMonumentDoubleXYRoom;
class OceanMonumentDoubleYRoom;
class OceanMonumentDoubleYZRoom;
class OceanMonumentDoubleZRoom;
class OceanMonumentEntryRoom;
class OceanMonumentFeature;
class OceanMonumentPenthouse;
class OceanMonumentPiece;
class OceanMonumentSimpleRoom;
class OceanMonumentSimpleTopRoom;
class OceanMonumentStart;
class OceanMonumentWingRoom;
class OceanRuinFeature;
namespace OceanRuinPieces {
    class OceanRuinPiece;
}
class OceanRuinStart;
class Ocelot;
class OcelotAttackGoal;
class OcelotSitOnBlockGoal;
class OfferFlowerGoal;
class OffhandContainerController;
class OldLeafBlock;
class OldLogBlock;
class OnDeathDescription;
class OnFriendlyAngerDescription;
class OnHitSubcomponent;
class OnHurtByPlayerDescription;
class OnHurtDescription;
class OnIgniteDescription;
class OnScreenTextureAnimationPacket;
class OnStartLandingDescription;
class OnStartTakeoffDescription;
class OnTargetAcquiredDescription;
class OnTargetEscapeDescription;
class OnWakeWithOwnerDescription;
class OpCommand;
class OpenDoorAnnotationComponent;
class OpenDoorAnnotationDescription;
class OpenDoorAnnotationSystem;
class OpenDoorGoal;
class Option;
class OptionLock;
class OptionObserver;
class OptionalString;
class OreBlock;
class OreFeature;
class OutputContainerController;
class OverloadSyntaxInformation;
class OverworldDimension;
class OverworldGenerator;
class OverworldHeightAttributes;
class OwnerHurtByTargetGoal;
class OwnerHurtTargetGoal;
class OwnerStorageEntity;
class OwnerStorageFeature;
class POIBlueprint;
class POIInstance;
class Pack;
class PackAccessStrategy;
class PackDiscoveryError;
class PackError;
class PackIdVersion;
class PackInstance;
class PackInstanceId;
class PackLoadError;
class PackManifest;
class PackManifestFactory;
class PackMetadata;
class PackRenderCapabilitiesBitSet;
class PackReport;
class PackSetting;
class PackSettingObserver;
class PackSettings;
class PackSettingsError;
class PackSettingsFactory;
class PackSource;
class PackSourceFactory;
class PackSourceReport;
class PackStats;
class Packet;
class PacketHeader;
class PacketObserver;
class PacketSender;
class PageContent;
class Painting;
class Panda;
class PanicGoal;
class Parrot;
class Particle;
class ParticleCommand;
class ParticleOnHitSubcomponent;
class ParticlesBlockExplosionEvent;
class ParticlesTeleportTrailEvent;
class Path;
class PathFinder;
class PathNavigation;
class PathfinderNode;
class PatternEntry;
class PeekComponent;
class PeekDefinition;
class PeekSystem;
class PendingArea;
class PendingRemovalInfo;
class PerfTimer;
class PerlinNoise;
class PerlinSimplexNoise;
class PermissionCommand;
class PermissionsFile;
class PermissionsHandler;
class PersistentDescription;
class PetSleepWithOwnerGoal;
class Phantom;
class PhotoStorage;
class PhotoTransferPacket;
class PhysicsComponent;
class PhysicsDefinition;
class PickaxeItem;
class PickupItemsGoal;
class PieceWeight;
class Pig;
class PigZombie;
class Pillager;
class PillagerOutpostFeature;
namespace PillagerOutpostPieces {
    class PillagerOutpostPiece;
}
class PillagerOutpostStart;
class PineFeature;
class PingedCompatibleServer;
class PistonArmBlock;
class PistonBlock;
class PistonBlockActor;
class PistonConsumer;
class PlaceBlockDefinition;
class PlaceBlockNode;
class PlanksBlock;
class PlayGoal;
class PlaySoundCommand;
class PlaySoundPacket;
class PlaySoundReactionComponent;
class PlayStatusPacket;
class Player;
class PlayerActionPacket;
class PlayerAuthInputPacket;
class PlayerCommandOrigin;
class PlayerEventCoordinator;
class PlayerEventListener;
class PlayerHotbarPacket;
class PlayerInputPacket;
class PlayerInteractionSystem;
class PlayerInventoryProxy;
class PlayerListEntry;
class PlayerListPacket;
class PlayerMovementTelemetryData;
class PlayerRespawnBlockRadiusRandomizer;
class PlayerRespawnRandomizer;
class PlayerRespawnTelemetryData;
class PlayerRideTamedGoal;
class PlayerScore;
class PlayerScoreboardId;
class PlayerSkinPacket;
class PlayerStorageIds;
class PlayerSuspension;
class PlayerUIContainer;
class PlayerUIContainerModel;
class PlayerUIContainerModelBase;
class PodzolAreaFeature;
class PodzolBlock;
class PolarBear;
class PoolElementStructurePiece;
class PortalBlock;
class PortalForcer;
class PortalRecord;
class PortalShape;
class Pos;
class PosibleTransformation;
class PostprocessingManager;
class PotatoBlock;
class Potion;
namespace PotionBrewing {
    class Ingredient;
}
class PotionItem;
class PoweredBlockComponent;
class PoweredRailBlock;
class PrecompiledCommandOrigin;
class PreferredPathComponent;
class PreferredPathDescription;
class PressurePlateBlock;
class PrimedTnt;
class PrioritizedGoal;
class PrismarineBlock;
class PrivateKeyManager;
class ProducerComponent;
class ProfilerLite;
class ProjectileComponent;
class ProjectileDescription;
class ProjectileFactory;
class ProjectileSystem;
class PropertiesSettings;
class PropertyBag;
class PropertyDescription;
class ProtectionEnchant;
class Pufferfish;
class PulseCapacitor;
class PumpkinBlock;
class PurchaseReceiptPacket;
class PushNotificationMessage;
class PushThroughDescription;
class PushableComponent;
class PushableDescription;
class QuartzBlockBlock;
class QueryTargetCommand;
class Rabbit;
class Raid;
class RaidBossComponent;
class RaidBossSystem;
class RaidGardenGoal;
class RaidTriggerComponent;
class RaidTriggerDescription;
class RaiderCelebrationGoal;
class RailActivatorDescription;
class RailActivatorSystem;
class RailBlock;
class RailMovementComponent;
class RailMovementDefinition;
class RailMovementSystem;
class RakDataInput;
class RakDataOutput;
namespace RakNet {
    class AddressOrGUID;
}
namespace RakNet {
    class BPSTracker;
}
namespace RakNet {
    class BitStream;
}
namespace RakNet {
    class CCRakNetSlidingWindow;
}
namespace RakNet {
    class HuffmanEncodingTree;
}
namespace RakNet {
    class IRNS2_Berkley;
}
namespace RakNet {
    class InternalPacket;
}
namespace RakNet {
    class InternalPacketFixedSizeTransmissionHeader;
}
namespace RakNet {
    class LocklessUint32_t;
}
namespace RakNet {
    class NetworkAdapter;
}
namespace RakNet {
    class Packet;
}
namespace RakNet {
    class PluginInterface2;
}
namespace RakNet {
    class RNS2EventHandler;
}
namespace RakNet {
    class RNS2RecvStruct;
}
namespace RakNet {
    class RNS2_Berkley;
}
namespace RakNet {
    class RNS2_Linux;
}
namespace RakNet {
    class RNS2_SendParameters;
}
namespace RakNet {
    class RNS2_Windows_Linux_360;
}
namespace RakNet {
    class RakNetGUID;
}
namespace RakNet {
    class RakNetRandom;
}
namespace RakNet {
    class RakNetSocket2;
}
namespace RakNet {
    class RakPeer;
}
namespace RakNet {
    class RakPeerInterface;
}
namespace RakNet {
    class RakString;
}
namespace RakNet {
    class ReliabilityLayer;
}
namespace RakNet {
    class RemoteClient;
}
namespace RakNet {
    class SignaledEvent;
}
namespace RakNet {
    class SimpleMutex;
}
namespace RakNet {
    class SocketDescriptor;
}
namespace RakNet {
    class SortedSplittedPackets;
}
namespace RakNet {
    class SplitPacketChannel;
}
namespace RakNet {
    class StringCompressor;
}
namespace RakNet {
    class StringTable;
}
namespace RakNet {
    class SystemAddress;
}
namespace RakNet {
    class TCPInterface;
}
namespace RakNet {
    class uint24_t;
}
class RakNetInstance;
class RakNetServerLocator;
class RakPeerHelper;
class RakStringCleanup;
class Random;
class RandomAuxValueFunction;
class RandomBlockStateFunction;
class RandomBreachingGoal;
class RandomFlyingGoal;
class RandomHoverGoal;
class RandomLookAroundAndSitGoal;
class RandomLookAroundGoal;
class RandomScatteredLargeFeature;
class RandomSitGoal;
class RandomStrollGoal;
class RandomSwimmingGoal;
class RandomThreadCheckManager;
class RandomValueBounds;
class RandomizableBlockActorContainer;
class RandomizableBlockActorContainerBase;
class RandomizableBlockActorFillingContainer;
class RangedAttackGoal;
class RangedWeaponItem;
class RapidFertilizerItem;
class RateLimiter;
class ReadOnlyBinaryStream;
class RealmsUnknownPackSource;
class ReceiveLoveGoal;
class Recipe;
class RecipeIngredient;
class Recipes;
class RecordItem;
class RedStoneDustItem;
class RedStoneOreBlock;
class RedStoneWireBlock;
class RedstoneBlock;
class RedstoneLampBlock;
class RedstoneTorchBlock;
class RedstoneTorchCapacitor;
class ReedBlock;
class ReedsFeature;
class RegionFile;
class RegionHillsLayer;
class RelativeFloat;
class ReloadCommand;
class RemoveActorPacket;
class RemoveEntityPacket;
class RemoveObjectivePacket;
class RemoveOnHitSubcomponent;
class RemovedActorDamageByType;
class RenderParams;
class RepairItemRecipe;
class RepeatUntilFailureDefinition;
class RepeatUntilFailureNode;
class RepeatUntilSuccessDefinition;
class RepeatUntilSuccessNode;
class RepeaterBlock;
class RepeaterCapacitor;
class ReplaceItemCommand;
class RequestChunkRadiusPacket;
class ResetEventObj;
class ResourceInformation;
class ResourceLoadManager;
class ResourceLoader;
class ResourceLocation;
class ResourceLocationPair;
class ResourceMetadata;
class ResourcePack;
class ResourcePackChunkDataPacket;
class ResourcePackChunkRequestPacket;
class ResourcePackClientResponsePacket;
class ResourcePackContents;
class ResourcePackDataInfoPacket;
class ResourcePackFileDownloaderManager;
class ResourcePackFileUploadManager;
class ResourcePackInfoData;
class ResourcePackListener;
class ResourcePackManager;
class ResourcePackMergeStrategy;
class ResourcePackRepository;
class ResourcePackStack;
class ResourcePackStackPacket;
class ResourcePackTransmissionManager;
class ResourcePacksInfoData;
class ResourcePacksInfoPacket;
class ResourcePath;
class ResourceSignature;
class RespawnPacket;
class RestrictOpenDoorGoal;
class RestrictSunGoal;
class ReverseableIterator;
class RideableComponent;
class RideableDescription;
class RiderJumpPacket;
class RiverFollowingGoal;
class RoleChecker;
class RoleCheckerCallback;
class RollGoal;
class RoofTreeFeature;
class RoomDefinition;
class RopeAABB;
class RopeNode;
class RopeParams;
class RopePoint;
class RopePoints;
class RopePointsRef;
class RopeSystem;
class RotatedPillarBlock;
class RotationDescription;
class RunAroundLikeCrazyGoal;
class RuntimeIdentifierDescription;
class RuntimeLightingManager;
class SHChestCorridor;
class SHFillerCorridor;
class SHFiveCrossing;
class SHLeftTurn;
class SHLibrary;
class SHPortalRoom;
class SHPrisonHall;
class SHRightTurn;
class SHRoomCrossing;
class SHStairsDown;
class SHStartPiece;
class SHStraight;
class SHStraightStairsDown;
class Salmon;
class SandBlock;
class SandFeature;
class SandStoneBlock;
class Sapling;
class SaplingBlockItem;
class SavannaTreeFeature;
class SaveCommand;
class SaveTransactionManager;
class SavedData;
class SavedDataStorage;
class SayCommand;
class ScaffoldingBlock;
class ScaffoldingBlockItem;
class ScaffoldingClimberComponent;
class ScaffoldingClimberSystem;
class ScaleByAgeComponent;
class ScaleByAgeDefinition;
class ScaleByAgeSystem;
class ScaleDescription;
class ScaredGoal;
class ScatterFeature;
class ScatterParams;
class ScatteredFeaturePiece;
class ScatteredFeatureStart;
class ScheduledCallback;
class Scheduler;
class SchedulerComponent;
class SchedulerDefinition;
class SchedulerSystem;
class ScopedAutoreleasePool;
class ScopedProfileTag;
class ScorePacketInfo;
class Scoreboard;
class ScoreboardCommand;
class ScoreboardId;
class ScoreboardIdentityPacketInfo;
class ScoreboardIdentityRef;
class ScriptActorAreaBinderComponent;
class ScriptActorUniqueIdBinderComponent;
namespace ScriptApi {
    class EmptyScriptInterface;
}
namespace ScriptApi {
    class ScriptCallbackInterface;
}
namespace ScriptApi {
    class ScriptFramework;
}
namespace ScriptApi {
    class ScriptLanguageInterface;
}
namespace ScriptApi {
    class ScriptObjectHandle;
}
namespace ScriptApi {
    class ScriptReport;
}
namespace ScriptApi {
    class ScriptReportItem;
}
namespace ScriptApi {
    class ScriptSystemInfo;
}
namespace ScriptApi {
    class ScriptVersionInfo;
}
namespace ScriptApi {
    namespace WORKAROUNDS {
        class tempActorComponent;
    }
}
namespace ScriptApi {
    namespace WORKAROUNDS {
        class tempLevelComponent;
    }
}
class ScriptArmorContainerComponent;
class ScriptAttackComponent;
class ScriptBinderActorTemplate;
class ScriptBinderActorTickingAreaTemplate;
class ScriptBinderBlockTemplate;
class ScriptBinderComponent;
class ScriptBinderComponentTemplate;
class ScriptBinderEventDataTemplate;
class ScriptBinderItemActorTemplate;
class ScriptBinderItemStackTemplate;
class ScriptBinderLevelTemplate;
class ScriptBinderLevelTickingAreaTemplate;
class ScriptBinderPureEcsTemplate;
class ScriptBinderQueryTemplate;
class ScriptBinderTemplate;
class ScriptBinderTemplateController;
class ScriptBlockContainerComponent;
class ScriptBlockPositionBinderComponent;
class ScriptBlockStateComponent;
class ScriptCollisionBoxComponent;
class ScriptCommand;
class ScriptCommandCallbackData;
class ScriptCommandOrigin;
class ScriptComponentBinderComponent;
class ScriptContainerComponentBase;
class ScriptCustomEventPacket;
class ScriptDamageSensorComponent;
class ScriptEcsBinderComponent;
class ScriptEngine;
class ScriptEquipmentComponent;
class ScriptEquippableComponent;
class ScriptEventCoordinator;
class ScriptEventData;
class ScriptEventDataBinderComponent;
class ScriptEventListener;
class ScriptExplodeComponent;
class ScriptHandContainerComponent;
class ScriptHealableComponent;
class ScriptHealthComponent;
class ScriptHotbarContainerComponent;
class ScriptIdentifierBinderComponent;
class ScriptInteractComponent;
class ScriptInventoryComponent;
class ScriptInventoryContainerComponent;
class ScriptItemStackBinderComponent;
class ScriptLevelAreaBinderComponent;
class ScriptLevelBinderComponent;
class ScriptLevelTickingAreasComponent;
class ScriptLevelWeatherComponent;
class ScriptLevelWeatherEventListener;
class ScriptLoggerConfig;
class ScriptLookAtComponent;
class ScriptNameableComponent;
class ScriptObjectBinder;
class ScriptPositionComponent;
class ScriptQueries;
class ScriptQueryBinderComponent;
class ScriptQueryComponent;
class ScriptRotationComponent;
class ScriptServerActorAcquiredItemEvent;
class ScriptServerActorAttackEvent;
class ScriptServerActorCarriedItemChangedEvent;
class ScriptServerActorCreatedEvent;
class ScriptServerActorDeathEvent;
class ScriptServerActorDefinitionEventTriggeredEvent;
class ScriptServerActorDroppedItemEvent;
class ScriptServerActorEquippedArmorEvent;
class ScriptServerActorEventListener;
class ScriptServerActorHurtEvent;
class ScriptServerActorMoveEvent;
class ScriptServerActorRemovedEvent;
class ScriptServerActorSneakChangedEvent;
class ScriptServerActorStartRidingEvent;
class ScriptServerActorStopRidingEvent;
class ScriptServerActorTickEvent;
class ScriptServerActorUseItemEvent;
class ScriptServerBlockDestructionStartedEvent;
class ScriptServerBlockDestructionStoppedEvent;
class ScriptServerBlockEventListener;
class ScriptServerBlockExplodedEvent;
class ScriptServerBlockInteractedWithEvent;
class ScriptServerBlockMovedByPistonEvent;
class ScriptServerBroadcastActorDefinitionEvent;
class ScriptServerBroadcastReceived;
class ScriptServerChatReceived;
class ScriptServerCommandReceived;
class ScriptServerContext;
class ScriptServerLevelEventListener;
class ScriptServerLoggerConfigReceivedEvent;
class ScriptServerPacketEventListener;
class ScriptServerPlaySoundEvent;
class ScriptServerPlayerAttackedActorEvent;
class ScriptServerPlayerDestroyedBlockEvent;
class ScriptServerPlayerPlacedBlockEvent;
class ScriptServerProjectileHitEvent;
class ScriptServerSpawnParticleAttachedToActor;
class ScriptServerSpawnParticleInWorldEvent;
class ScriptServerWeatherEventData;
class ScriptShooterComponent;
class ScriptSpawnActorComponent;
class ScriptTagComponent;
class ScriptTelemetryEventListener;
class ScriptTeleportComponent;
class ScriptTickWorldComponent;
class ScriptTickingAreaBinderComponent;
class ScriptTickingAreaDescriptionComponent;
class SeaAnemoneFeature;
class SeaGrass;
class SeaLanternBlock;
class SeaPickle;
class SeaPickleBlockItem;
class SeaPickleFeature;
class SeagrassFeature;
class Seasons;
class SeatDescription;
class SecureStorage;
class SecureStorageKey;
class SeedItemComponent;
class SelectorBehaviorNode;
class SelectorDefinition;
class SemVersion;
class Semaphore;
class SendEventData;
class SendEventGoal;
class SendEventStage;
class SensingComponent;
class SensingSystem;
class SequenceBehaviorNode;
class SequenceDefinition;
class SerializedSkin;
class ServerCommand;
class ServerCommandOrigin;
class ServerCommunicationInterface;
class ServerContentKeyProvider;
class ServerFileChunkUploader;
class ServerInstance;
class ServerInstanceEventCoordinator;
class ServerInstanceEventListener;
class ServerLevel;
class ServerLevelEventCoordinator;
class ServerLocator;
class ServerMetrics;
class ServerMetricsImpl;
class ServerMoveInputHandler;
class ServerNetworkHandler;
class ServerPlayer;
class ServerPlayerEventCoordinator;
class ServerScoreboard;
class ServerSettingsRequestPacket;
class ServerSettingsResponsePacket;
class ServerToClientHandshakePacket;
class SetActorDataPacket;
class SetActorLinkPacket;
class SetActorMotionPacket;
class SetBannerDetailsFunction;
class SetBlockCommand;
class SetBlockReactionComponent;
class SetBookContentsFunction;
class SetCommandsEnabledPacket;
class SetDataFromColorIndexFunction;
class SetDefaultGameTypePacket;
class SetDifficultyPacket;
class SetDisplayObjectivePacket;
class SetHealthPacket;
class SetItemCountFunction;
class SetItemDamageFunction;
class SetItemDataFunction;
class SetItemLoreFunction;
class SetItemNameFunction;
class SetLastHurtByPacket;
class SetLocalPlayerAsInitializedPacket;
class SetMaxPlayersCommand;
class SetPlayerGameTypePacket;
class SetScorePacket;
class SetScoreboardIdentityPacket;
class SetSpawnEggFunction;
class SetSpawnPositionPacket;
class SetTimePacket;
class SetTitlePacket;
class SetWorldSpawnCommand;
class SettingsCommandPacket;
class ShapedChemistryRecipe;
class ShapedRecipe;
class ShapelessChemistryRecipe;
class ShapelessRecipe;
class ShareItemsGoal;
class Shareable;
class ShareableComponent;
class ShareableDefinition;
class ShearsItem;
class Sheep;
class ShieldItem;
class ShiftedDurationAmplifier;
class ShiftedValueAmplifier;
class ShipwreckFeature;
class ShipwreckPiece;
class ShipwreckStart;
class ShootBowDefinition;
class ShootBowNode;
class ShooterComponent;
class ShooterDescription;
class ShortTag;
class ShovelItem;
class ShowCreditsPacket;
class ShowProfilePacket;
class ShowStoreOfferPacket;
class Shulker;
class ShulkerBoxBlock;
class ShulkerBoxBlockActor;
class ShulkerBoxBlockItem;
class ShulkerBoxContainerController;
class ShulkerBoxRecipe;
class ShulkerBullet;
class ShulkerPeekGoal;
class SignBlock;
class SignBlockActor;
class SignItem;
class Silverfish;
class SilverfishMergeWithStoneGoal;
class SilverfishWakeUpFriendsGoal;
class SimpleBoolFilterTest;
class SimpleContainer;
class SimpleEventPacket;
class SimpleFloatFilterTest;
class SimpleHashStringFilterTest;
class SimpleIntFilterTest;
class SimpleTagIDFilterTest;
class SimplexNoise;
class SingleBlockFeature;
class SitComponent;
class SitGoal;
class SittableDescription;
class Skeleton;
class SkeletonHorseTrapGoal;
class SkinAdjustments;
class SkinData;
class SkinIDDescription;
class SkinInfoData;
class SkullBlock;
class SkullBlockActor;
class SkullItem;
class SlabBlock;
class SlabBlockItem;
class SleepGoal;
class Slime;
class SlimeAttackGoal;
class SlimeBlock;
class SlimeFloatGoal;
class SlimeKeepOnJumpingGoal;
class SlimeMoveControl;
class SlimeMoveControlDescription;
class SlimeRandomDirectionGoal;
class SlotData;
class SlotDescriptor;
class SlotDropChance;
class SmallFireball;
class SmeltItemFunction;
class SmokerBlock;
class SmokerBlockActor;
class SmokerContainerManagerModel;
class SmoothStoneSelector;
class SnackGoal;
class SnapshotEnv;
class SnapshotFilenameAndLength;
class SnapshotWritableFile;
class SneezeGoal;
class SnowBlock;
class Snowball;
class SnowballItem;
namespace Social {
    namespace Events {
        class Event;
    }
}
namespace Social {
    namespace Events {
        class EventManager;
    }
}
namespace Social {
    namespace Events {
        class Measurement;
    }
}
namespace Social {
    namespace Events {
        class Property;
    }
}
namespace Social {
    class GameConnectionInfo;
}
namespace Social {
    class MultiplayerServiceObserver;
}
namespace Social {
    class XboxLiveSignInHandler;
}
namespace Social {
    class XboxLiveUserObserver;
}
class SoulSandBlock;
class SoundPlayer;
class SoundVolumeDescription;
class SparklerItem;
class SpatialActorNetworkData;
class SpawnActorComponent;
class SpawnActorDescription;
class SpawnActorEntry;
class SpawnActorParameters;
class SpawnActorSystem;
class SpawnAoECloudSubcomponent;
class SpawnChanceSubcomponent;
class SpawnConditions;
class SpawnData;
class SpawnExperienceOrbPacket;
class SpawnGroupData;
class SpawnGroupRegistry;
class SpawnParticleEffectPacket;
class SpawnPointCommand;
class Spawner;
class SpecificEnchantFunction;
class Spider;
class SpikeFeature;
class SpinLock;
class SplashPotionEffectSubcomponent;
class SplashPotionItem;
class SpongeBlock;
class SpreadPlayersCommand;
class SpringFeature;
class SpruceFeature;
class Squid;
class SquidDiveGoal;
class SquidFleeGoal;
class SquidIdleGoal;
class SquidMoveAwayFromGroundGoal;
class SquidOutOfWaterGoal;
class StackResultStorageEntity;
class StackResultStorageFeature;
class StackStats;
class StackedGraphBars;
class StainedGlassBlock;
class StainedGlassPaneBlock;
class StairBlock;
class StalkAndPounceOnTargetGoal;
class StartGamePacket;
class StateAnimationVariable;
class StateVectorComponent;
class StemBlock;
class StickInGroundSubcomponent;
class StompAttackGoal;
class StompBlockGoal;
class StompEggGoal;
class StoneBlock;
class StoneBrickBlock;
class StoneButtonBlock;
class StoneSlabBlock;
class StoneSlabBlock2;
class StoneSlabBlock3;
class StoneSlabBlock4;
class StoneSlabBlockItem;
class StonecutterBlock;
class StopCommand;
class StopSoundCommand;
class StopSoundPacket;
class Stopwatch;
class StopwatchNLast;
class StrengthDescription;
class StringByteInput;
class StringByteOutput;
class StringKey;
class StringOption;
class StringTag;
class StrippedLogBlock;
class StrollTowardsVillageGoal;
class StrongholdFeature;
class StrongholdPiece;
class StrongholdStart;
class StructureBlock;
class StructureBlockActor;
class StructureBlockPalette;
class StructureBlockUpdatePacket;
class StructureDataLoadHelper;
class StructureEditorData;
class StructureFeature;
class StructureIntegrityProcessor;
class StructureManager;
class StructurePiece;
class StructurePoolActorPredicateActorMatch;
class StructurePoolActorPredicateAlwaysTrue;
class StructurePoolActorRule;
class StructurePoolBlockPredicateAlwaysTrue;
class StructurePoolBlockPredicateBlockMatch;
class StructurePoolBlockPredicateBlockMatchRandom;
class StructurePoolBlockPredicateStateMatch;
class StructurePoolBlockPredicateStateMatchRandom;
class StructurePoolBlockRule;
class StructurePoolBlockTagPredicateAlwaysTrue;
class StructurePoolBlockTagPredicateBlockTagMatch;
class StructurePoolBlockTagPredicateBlockTagStringMatches;
class StructurePoolBlockTagRule;
class StructurePoolElement;
class StructureSettings;
class StructureStart;
class StructureTelemetryClientData;
class StructureTelemetryServerData;
class StructureTemplate;
class StructureTemplateData;
class StructureTemplateDataRequestPacket;
class StructureTemplateDataResponsePacket;
class StructureTemplateFeature;
class StructureTemplatePool;
class StructureVoid;
class SubChunk;
class SubChunkBlockPos;
class SubChunkBlockStorage;
class SubChunkBrightnessStorage;
class SubChunkLightIndex;
class SubChunkLightUpdate;
class SubChunkRelighter;
class SubChunkStorageFormat;
class SubClientConnectionRequest;
class SubClientLoginPacket;
class SubpackInfo;
class SubpackInfoCollection;
class SubtractiveLightInfo;
class SubtreeDefinition;
class SubtreeNode;
class SummonActorGoal;
class SummonCommand;
class SummonSpellData;
class SummonSpellStage;
class SurfaceBuilderComponent;
class SurfaceBuilderRegistry;
class SurfaceMaterialAdjustmentAttributes;
class SurfaceMaterialAttributes;
class SurvivalMode;
class SuspiciousStewItem;
class SwampTreeFeature;
class SwamplandHut;
class SweetBerryBushBlock;
class SwellGoal;
class SwimEnchant;
class SwimIdleGoal;
class SwimMoveControl;
class SwimWanderGoal;
class SwimWithMobGoal;
class SwoopAttackGoal;
class SyncedAttribute;
class SynchedActorData;
class SystemFilePicker;
class Tag;
class TagCommand;
class TagMemoryChunk;
class TagRegistry;
class TagsComponent;
class TagsDescription;
class TakeFlowerGoal;
class TakeItemActorPacket;
class TallGrass;
class TallGrassFeature;
class TameableComponent;
class TameableDescription;
class TargetGoal;
class TargetNearbyComponent;
class TargetNearbyDescription;
class TargetNearbySystem;
class TargetWhenPushedGoal;
class TaskGroup;
class TaskResult;
class TaskStartInfo;
class TelemetryInfo;
class TeleportCommand;
class TeleportComponent;
class TeleportDescription;
class TeleportSystem;
class TeleportToSubcomponent;
class TellCommand;
class TellRawCommand;
class TemplateStructurePiece;
class TemporalAttributeBuff;
class TemptGoal;
class TerrainBurstReactionComponent;
class TestCommandOrigin;
class TestForBlockCommand;
class TestForBlocksCommand;
class TestForCommand;
class TextObjectLocalizedText;
class TextObjectLocalizedTextWithParams;
class TextObjectRoot;
class TextObjectText;
class TextPacket;
class TextureAtlasItem;
class TextureItem;
class TextureUVCoordinateSet;
class TheEndDimension;
class TheEndGenerator;
class ThinFenceBlock;
class ThirdPartyInfo;
class ThirdPartyServer;
class ThreadConfiguration;
class ThreadSpecificData;
class Throwable;
class ThrownEgg;
class ThrownEnderpearl;
class ThrownIceBomb;
class ThrownPotion;
class ThrownPotionEffectSubcomponent;
class ThrownTrident;
class Tick;
class TickNextTickData;
class TickSyncPacket;
class TickWorldComponent;
class TickWorldDescription;
class TickingArea;
class TickingAreaCommand;
class TickingAreaDescription;
class TickingAreaList;
class TickingAreaListBase;
class TickingAreaView;
class TickingAreasManager;
class TimeCommand;
class Timer;
class TimerComponent;
class TimerDescription;
class TimerSystem;
class TitleCommand;
class TitleRawCommand;
class TntBlock;
class ToggleDownfallCommand;
class Token;
class TopSnowBlock;
class TopSnowBlockItem;
class TorchBlock;
class TrackerStat;
class Trade;
class TradeContainerManagerModel;
class TradeGroup;
class TradeIngredientContainerController;
class TradeInterestGoal;
class TradeItem;
class TradeResultContainerController;
class TradeResupplyComponent;
class TradeResupplyDescription;
class TradeTable;
class TradeTables;
class TradeTier;
class TradeWithPlayerGoal;
class TradeableSystem;
class TrailComponent;
class TrailDescription;
class TrailSystem;
class TransferPacket;
class TransformationComponent;
class TransformationDescription;
class TransformationSystem;
class TransporterComponent;
class TrapDoorBlock;
class TreatmentPackSource;
class TreeFeature;
class TridentChannelingEnchant;
class TridentImpalerEnchant;
class TridentItem;
class TridentLoyaltyEnchant;
class TridentRiptideEnchant;
class TriggerDescription;
class TripWireBlock;
class TripWireHookBlock;
class TripodCamera;
class TripodCameraComponent;
class TripodCameraDescription;
class TropicalFish;
class TropicalFishInfo;
class TrustComponent;
class TrustDescription;
class TrustingComponent;
class TrustingDescription;
class Turtle;
class TurtleEggBlock;
class UIPackError;
class UIProfanityContext;
class UPNPInterface;
class UPNPResult;
class UnburiedConstraint;
class UnderwaterCanyonFeature;
class UnderwaterLargeCaveFeature;
class UnderwaterTorchBlock;
class UndyedShulkerBoxBlock;
class UnverifiedCertificate;
class UpdateAttributesPacket;
class UpdateBlockPacket;
class UpdateBlockPropertiesPacket;
class UpdateBlockSyncedPacket;
class UpdateEquipPacket;
class UpdateSoftEnumPacket;
class UpdateTradePacket;
class UriListener;
class UseActorDefinition;
class UseActorNode;
namespace Util {
    class HashString;
}
class VanillaAppConfigs;
class VanillaGameModuleDedicatedServer;
class VanillaGameModuleServer;
class VanillaInPackagePacks;
class VanillaOverworldBiomeSource;
class VanillaServerGameplayEventListener;
namespace VanillaSurfaceBuilders {
    class MesaSurfaceBuilder;
}
namespace VanillaSurfaceBuilders {
    class NetherSurfaceBuilder;
}
namespace VanillaSurfaceBuilders {
    class OceanFrozenSurfaceBuilder;
}
namespace VanillaSurfaceBuilders {
    class OverworldDefaultSurfaceBuilder;
}
namespace VanillaSurfaceBuilders {
    class SwampSurfaceBuilder;
}
namespace VanillaSurfaceBuilders {
    class TheEndSurfaceBuilder;
}
class VariantDescription;
class VariantParameterList;
class Vec2;
class Vec3;
class Vec3Option;
class Vex;
class VexCopyOwnerTargetGoal;
class VexRandomMoveGoal;
class VideoStreamConnectPacket;
class VideoStreamServerCommand;
class Village;
class VillageFeature;
class VillageManager;
class VillagePiece;
class VillageStart;
class Villager;
class VillagerBase;
class VillagerV2;
class VindicationIllager;
class VineBlock;
class VinesFeature;
class VirtualCommandOrigin;
class WASDControlledDescription;
class WSAStartupSingleton;
class WSServerCommand;
class WaitDefinition;
class WaitNode;
class WaitTicksDefinition;
class WaitTicksNode;
class WalkAnimationSpeedDescription;
class WallBlock;
class WallClimberPathNavigation;
class WanderingTrader;
class WanderingTraderScheduler;
class WantsJockeyDescription;
class WaterAnimal;
class WaterBoundPathNavigation;
class WaterLilyBlockItem;
class WaterMovementComponent;
class WaterMovementDescription;
class WaterlilyBlock;
class WaterlilyFeature;
class WeakStorageEntity;
class WeakStorageFeature;
class WeaponItem;
class Weather;
class WeatherCommand;
class WebBlock;
class WebSocketPacketData;
class WebToken;
class WebviewObserver;
namespace WeighedRandom {
    class WeighedRandomItem;
}
class WeightedPressurePlateBlock;
class WeightedRandomFeature;
class WellKnownBiomeTags;
class WellKnownTagID;
class Whitelist;
class WhitelistCommand;
class WhitelistEntry;
class WhitelistFile;
class WireframeQueue;
class Witch;
class WitherBoss;
class WitherDoNothingGoal;
class WitherRandomAttackPosGoal;
class WitherRoseBlock;
class WitherSkull;
class WitherTargetHighestDamage;
class Wolf;
class WoodBlock;
class WoodButtonBlock;
class WoodSlabBlock;
class WoodSlabBlockItem;
class WoodlandMansionFeature;
namespace WoodlandMansionPieces {
    class FirstFloorRoomCollection;
}
namespace WoodlandMansionPieces {
    class FloorRoomCollection;
}
namespace WoodlandMansionPieces {
    class MansionGrid;
}
namespace WoodlandMansionPieces {
    class MansionPiecePlacer;
}
namespace WoodlandMansionPieces {
    class PlacementData;
}
namespace WoodlandMansionPieces {
    class SecondFloorRoomCollection;
}
namespace WoodlandMansionPieces {
    class SimpleGrid;
}
namespace WoodlandMansionPieces {
    class ThirdFloorRoomCollection;
}
namespace WoodlandMansionPieces {
    class WoodlandMansionPiece;
}
class WoodlandMansionStart;
class WoolCarpetBlock;
class WorkGoal;
class WorkbenchBlock;
class WorkerConfiguration;
class WorkerPool;
class WorldBlockTarget;
class WorldBuilderCommand;
class WorldChangeTransaction;
class WorldGenClimateMappingAttributes;
class WorldGenerator;
class WorldHistoryPackSource;
class WorldLimitChunkSource;
class WorldPackHistory;
class WorldPacksHistoryFile;
class WorldSessionEndPoint;
class WorldTemplateInfo;
class WorldTemplateLevelData;
class WorldTemplateManager;
class WorldTemplateManagerProxy;
class WorldTemplateManagerProxyCallbacks;
class WorldTemplatePackManifest;
class WorldTemplatePackSource;
class WritableBookItem;
class WrittenBookItem;
class XPCommand;
class ZipPackAccessStrategy;
class ZipPackAccessStrategyOwningFileAcccess;
class ZlibFileAccessWrapper;
class Zombie;
class ZombieVillager;
class _TickPtr;
class batch_control;
namespace com {
    namespace mojang {
        namespace clacks {
            namespace protocol {
                namespace Commands {
                    class Service;
                }
            }
        }
    }
}
namespace com {
    namespace mojang {
        namespace clacks {
            namespace protocol {
                namespace Commands {
                    class Stub;
                }
            }
        }
    }
}
namespace com {
    namespace mojang {
        namespace clacks {
            namespace protocol {
                namespace Commands {
                    class StubInterface;
                }
            }
        }
    }
}
namespace com {
    namespace mojang {
        namespace clacks {
            namespace protocol {
                class Empty;
            }
        }
    }
}
namespace com {
    namespace mojang {
        namespace clacks {
            namespace protocol {
                class LevelFileAndSize;
            }
        }
    }
}
namespace com {
    namespace mojang {
        namespace clacks {
            namespace protocol {
                class Message;
            }
        }
    }
}
namespace com {
    namespace mojang {
        namespace clacks {
            namespace protocol {
                class MetricReport;
            }
        }
    }
}
namespace com {
    namespace mojang {
        namespace clacks {
            namespace protocol {
                class MetricReport_BandwithMetric;
            }
        }
    }
}
namespace com {
    namespace mojang {
        namespace clacks {
            namespace protocol {
                class MetricReport_LatencyMetric;
            }
        }
    }
}
namespace com {
    namespace mojang {
        namespace clacks {
            namespace protocol {
                class PlayerAndMessage;
            }
        }
    }
}
namespace com {
    namespace mojang {
        namespace clacks {
            namespace protocol {
                class PlayerInfo;
            }
        }
    }
}
namespace com {
    namespace mojang {
        namespace clacks {
            namespace protocol {
                class PlayerList;
            }
        }
    }
}
namespace com {
    namespace mojang {
        namespace clacks {
            namespace protocol {
                class PlayerName;
            }
        }
    }
}
namespace com {
    namespace mojang {
        namespace clacks {
            namespace protocol {
                class SaveQueryResult;
            }
        }
    }
}
namespace com {
    namespace mojang {
        namespace clacks {
            namespace protocol {
                class SaveStateResult;
            }
        }
    }
}
namespace com {
    namespace mojang {
        namespace clacks {
            namespace protocol {
                class Settings;
            }
        }
    }
}
namespace com {
    namespace mojang {
        namespace clacks {
            namespace protocol {
                class Xuid;
            }
        }
    }
}
namespace commands {
    class Postfix;
}
namespace commands {
    class SoftEnum;
}
class gpr_arena;
namespace mce {
    class Blob;
}
namespace mce {
    class Degree;
}
namespace mce {
    class Image;
}
namespace mce {
    class Radian;
}
namespace mce {
    class UUID;
}
namespace moodycamel {
    namespace details {
        class ConcurrentQueueProducerTypelessBase;
    }
}
class parent_call;
namespace protobuf_main_2fproto_2fshared_2eproto {
    class StaticDescriptorInitializer;
}
class start_timer_after_init_state;
class zone;
